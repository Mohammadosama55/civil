...
router.get("/admin/dashboard", requireWardAdmin, async (req: Request, res: Response): Promise<void> => {
  ...
  res.json({
    ...
    categories: categoryAgg.map((c: { _id: string; count: number }) => ({ name: c._id, count: c.count })),
...
router.patch("/admin/issues/:id/status", requireWardAdmin, async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params;
  ...
  if (admin.ward && (issue as any).ward && (issue as any).ward !== admin.ward) {
    res.status(403).json({ error: "This issue is not in your ward" });
    return;
  }
...
router.get("/admin/issues/:id/pdf", requireWardAdmin, async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params;
  const issue = await Issue.findById(id).catch(() => null);
  if (!issue) { res.status(404).json({ error: "Issue not found" }); return; }
...
router.get("/admin/users", requireWardAdmin, async (_req: Request, res: Response): Promise<void> => {
...
router.patch("/admin/users/:id/role", requireWardAdmin, async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params;
